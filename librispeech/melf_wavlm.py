...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        mels = self.modules.compute_features(wavs)
        mels = self.modules.mean_var_norm(mels, wav_lens)
        projected_mels = self.modules.projector(mels)
        # Forward pass
        if self.thop : 
            macs, params = profile(self.modules.wav2vec2.model, inputs=(projected_mels, ), verbose =False)
            self.macs.append(macs)

        feats = self.modules.wav2vec2(projected_mels) # to be changed
