...
try:
    ...
    from transformers import eeWavLMConfig, eeWavLMModel
    from transformers import melWavLMConfig, melWavLMModel
...
HF_models = {
    ...
    "hubert": HubertModel,
    "wavlm": WavLMModel,
    "eewavlm": eeWavLMModel,
    "melwavlm": melWavLMModel
}
...
HF_config = {
    ...
    "hubert": HubertConfig,
    "wavlm": WavLMConfig,
    "eewavlm": eeWavLMConfig,
    "melwalm": melWavLMConfig
}
...
class HuggingFaceWav2Vec2(nn.Module):
    ...
    def __init__(
        ...
        if "hubert" in source:
            ...
            config = HF_config.get("eewavlm")
            model = HF_models.get("eewavlm")
            print("got the early exit config")
        elif "mel" in source: 
            config = HF_config.get("melwavlm")
            model = HF_models.get("melwavlm")
            print("got the mel config")
