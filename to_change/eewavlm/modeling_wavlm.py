...
from .configuration_wavlm import eeWavLMConfig
...
class WavLMEncoderLayer(nn.Module):
    def __init__(self, config: eeWavLMConfig, has_relative_position_bias: bool = True):
        super().__init__()
...
class WavLMEncoderStableLayerNorm(nn.Module):
    def __init__(self, config):
        ...
        self.early_exits = []
        for i in range(12): 
            new_linear =nn.Linear(1024,1024,bias= True).cuda() 
            self.early_exits.append(new_linear)
        self.ctc_decoders = []
        for i in range(12): 
            new_dec =nn.Linear(1024,31).cuda() 
            self.ctc_decoders.append(new_dec)
        self.confidence_mode = False
        self.similarity_mode = 1 - self.confidence_mode
        self.activation  = torch.nn.LeakyReLU()
        list_params = list(self.early_exits[0].parameters())
        for i in range(1,12):
            list_params +=list(self.early_exits[i].parameters())
        for i in range(0,12):

            list_params +=list(self.ctc_decoders[i].parameters())
        self.all_params = list_params
        self.layers = nn.ModuleList(
            [
        ...
        self.early_exit_mode = False
        self.gradient_checkpointing = False
        self.entropy_threshold = 2
        softmax_function = nn.Softmax(dim=2)
        def entropy_compute(ctc_output) :
            softmax = softmax_function(ctc_output)
            entropy = Categorical(probs =softmax).entropy()
            mean_entropy = torch.mean(entropy)
            return mean_entropy 
        cosine = nn.CosineSimilarity(dim=2)
        def sim_compute(hidden_states, prev_hs)  :
            return torch.mean(cosine(hidden_states, prev_hs))
        self.criterion = entropy_compute
        self.similarity_criterion = sim_compute
    ...
    def forward(
        ...
        for i, layer in enumerate(self.layers):
            ...
            # add LayerDrop (see https://arxiv.org/abs/1909.11556 for description)
            dropout_probability = np.random.uniform(0, 1)
            dropout_probability = 1.5 # added to block dropout here 
            ...
            if output_attentions:
                all_self_attentions = all_self_attentions + (layer_outputs[2],)
            if not self.early_exit_mode : 
                if i>11 : 
                    decoded = self.early_exits[i-12](hidden_states)
                    decoded = self.activation(decoded)
                    ctc_output = self.ctc_decoders[i-12](decoded)
                    ctc_outputs.append(ctc_output)

            if self.early_exit_mode : 
                if self.confidence_mode: 
                    if i>11:
                        ...
                        ctc_output = self.ctc_decoders[i-12](decoded)
                        ctc_outputs.append(ctc_output)
                        if self.criterion(ctc_output) < self.entropy_threshold or i ==23 :
                            self.out_layers.append(i)
                            break
                if self.similarity_mode: 

                    if i>11:
                        sim_value= self.similarity_criterion(hidden_states, previous_hidden_states) 
                        if sim_value > self.similarity_threshold or i==23: 
                            decoded = self.early_exits[i-12](hidden_states)
                            decoded = self.activation(decoded)
                            ctc_output = self.ctc_decoders[i-12](decoded)
                            ctc_outputs.append(ctc_output)
...
class eeWavLMModel(WavLMPreTrainedModel):
    def __init__(self, config: eeWavLMConfig):
        super().__init__(config)
        self.config = config
    ...
    def forward(
        ...
        encoder_outputs, ctc_outputs = self.encoder(
            hidden_states,
            attention_mask=attention_mask,
            output_attentions=output_attentions,
