...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        subsampled = self.modules.subsampling(wavs)
        # Forward pass
        if self.thop : 
            macs, params = profile(self.modules.wav2vec2.model, inputs=(subsampled, ), verbose =False)
            self.macs.append(macs)

        feats = self.modules.wav2vec2(subsampled)
