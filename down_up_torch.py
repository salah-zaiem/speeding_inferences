...
class ASR(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        logits  = self.modules.upsample(logits)
        logits = logits.view(logits.shape[0],-1,31)
...
if __name__ == "__main__":
    ...
    asr_brain.thop = False #We wont calculate the MAcs during training obviously
    ...
    asr_brain.resampler = T.Resample(16000, hparams["downsampling_rate"], dtype=torch.float).to(asr_brain.device)
    asr_brain.fit(
    ...
    # Now with thops, should be last because it crashes the following  
    asr_brain.device= "cuda:0"
    asr_brain.modules.to("cuda:0")
    asr_brain.resampler.to("cuda:0")
